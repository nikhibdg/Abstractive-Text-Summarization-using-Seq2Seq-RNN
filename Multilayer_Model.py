...
with tf.Session() as sess:
    ...
    # multi_rnn_cell = tf.nn.rnn_cell.BasicLSTMCell(num_units=128)

    rnn_layers = [tf.nn.rnn_cell.LSTMCell(size) for size in [64, 64]]
    ...
    _, decoder_initial_state = encoder_state
    ...
    for step in range(test_steps):  # with batch size 32 this will be 1k data points.
        pred, o_i, seq_lens = sess.run([train_prediction, label_index, labels_sequence_length], feed_dict=None)
        x = reverse_summary_vocab.lookup(tf.constant(pred, tf.int64))
