...
with tf.Session() as sess:
    ...
    forward_cell = tf.nn.rnn_cell.LSTMCell(64)
    backward_cell = tf.nn.rnn_cell.LSTMCell(64)

    bi_outputs, encoder_state = tf.nn.bidirectional_dynamic_rnn(
        forward_cell, backward_cell, embedded_inputs,dtype=tf.float64,
    ...
    #
    decoder_initial_state,_ = encoder_state
    #
    # ########################## Decoder #######################
    #
    ...
    infer_decoder = tf.contrib.seq2seq.BasicDecoder(
        decoder_cell, infer_helper, decoder_initial_state,
        output_layer=projection_layer)
    ...
    if os.path.exists('./tmp/bi_direction/'):
        saver.restore(sess, "./tmp/bi_direction/model.ckpt")
    else:
    ...
    for epoch in range(epochs):
        ...
        # saver = tf.train.Saver()
        save_path = saver.save(sess, "./tmp/bi_direction/model.ckpt")
