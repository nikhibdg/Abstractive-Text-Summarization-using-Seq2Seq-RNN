...
text_vocab = lookup_ops.index_table_from_file(
    "drive/ML_FINAL/feature_rich/features_text_vocab.txt", default_value=0)

summary_vocab = lookup_ops.index_table_from_file(
    "drive/ML_FINAL/feature_rich/features_summary_vocab.txt", default_value=0)

summary_vocab_size = np.loadtxt("drive/ML_FINAL/feature_rich/features_summary_vocab.txt", dtype="str").size

text_embedings = np.loadtxt("drive/ML_FINAL/feature_rich/features_text_vocab_embedings.txt");
text_emb_mat = tf.constant(text_embedings)

summary_embedings = np.loadtxt("drive/ML_FINAL/feature_rich/features_summary_vocab_embedings.txt");
summary_emb_mat = tf.constant(summary_embedings)

reverse_text_vocab = tf.contrib.lookup.index_to_string_table_from_file("drive/ML_FINAL/feature_rich/features_text_vocab.txt", default_value='<unk>')
reverse_summary_vocab = tf.contrib.lookup.index_to_string_table_from_file("drive/ML_FINAL/feature_rich/features_summary_vocab.txt", default_value='<unk>')

src_dataset = tf.data.TextLineDataset("drive/ML_FINAL/feature_rich/features_text.txt")
tgt_dataset = tf.data.TextLineDataset("drive/ML_FINAL/feature_rich/features_summary.txt")
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter("drive/ML_FINAL/fr_saved_model/2")
    ...
    for epoch in range(epochs):
        ...
        # saver = tf.train.Saver()
        save_path = saver.save(sess, "drive/ML_FINAL/fr_saved_model/model.ckpt".format(epoch))
